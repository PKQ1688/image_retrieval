...
def create_table_mysql(conn,cursor):
    sql = "create table if not exists " + DEFAULT_TABLE + "(milvus_id varchar(25), images_id varchar(30), index ix_milvus (milvus_id), index ix_images (images_id));"
    try:
        cursor.execute(sql)
        print("create table")
    except Exception as e:
...
def delete_data(conn, cursor, image_id):
    sql = "delete from " + DEFAULT_TABLE + " where images_id = '" + image_id + "';"
    try:
        cursor.execute(sql)
        conn.commit()
    except Exception as e:
